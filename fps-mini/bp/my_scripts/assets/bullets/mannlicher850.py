# -*- coding: utf-8 -*-
Asset = {
    'itemType': 'ww1:ammo_mannlicher850',

    # 弹道
    'projectile': {
        'baseSpeed': 620,               # 初速基准值 (米/秒), 最终初速 = 此值 + 枪管加成
        'gravity': -2,                  # 重力值（为了娱乐性不采用真实的重力）
        'drag': 0.005,                   # 空气阻力系数, 影响速度衰减 (0为无衰减)
        'caliber': 'mannlicher8x50',             # 口径标识 (string), 作为 customTag 传入 hurt 组件
    ...
    'payloads': [
        ...
        {
            'type': 'kinetic',                      # 载荷类型
            'baseDamage': 20.1,                       # 基础伤害值
            'headshotMultiplier': 1.8,              # 爆头伤害倍率
            'damageCurve': 'curves.mannlicher850'             # 伤害随速度衰减曲线资源ID (asset uri)
        },
        # 载荷2: 爆炸 (可选)
        # {
        #     'type': 'explosive',
