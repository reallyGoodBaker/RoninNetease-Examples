...
Asset = {
    ...
    'bolt': {
        ...
        'cuppedPortOpenTime': 0.84,    # 当膛内有实弹拉开枪机（退膛）需要的时间
        # 枪机打开/关闭动画
        ...
        'boltOpenTime': 0.32,        # 击发后枪机完全打开的时间
    ...
    'feed': {
        ...
        'ammoType': [ 'bullets.balleD' ],     # 兼容的弹药Asset列表
        'reloadModes': [
            {
                'condition': 'empty',           # 在子弹完全空了的时候换弹
                'reloadType': 'replace',        # 将子弹数替换到 magazineCapacity
                'reloadTime': 1,              # 完全换弹耗时(空仓或战术换弹), replace时为主要耗时
                'animation': 'reload_empty',                # 换弹动画
            },
            {
                'condition': 'not_full',           # 在子弹完全空了的时候换弹
                'reloadType': 'replace',        # 将子弹数替换到 magazineCapacity
                'reloadTime': 2,              # 完全换弹耗时(空仓或战术换弹), replace时为主要耗时
                'animation': 'reload_clip',                # 换弹动画
            },
        ]
    },
    ...
    'features': {
        'shoot': {
            'animation': 'shoot',
            'fireSound': 'berthier_m1916.shoot',       # 射击音效，空字符串为不播放
    ...
    'slots': [
        ...
        },
        {
            ...
            'attachmentAsset': 'attachments.appearance.berthier_m1916',
            'category': 'appearance_berthier_m1916',                   # 槽位分类, 自由字符串
