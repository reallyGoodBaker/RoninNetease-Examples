...
Asset = {
    ...
    'bolt': {
        ...
        # 手动循环模式相关：
        'shootRestoreTime': 0.25,    # 射击后到可以手拉枪机的时间
    ...
    'features': {
        'shoot': {
            'animation': 'fp.shoot',
            'fireSound': 'smle_mk3.shoot',       # 射击音效，空字符串为不播放
    ...
    'slots': [
        ...
        },
        {
            ...
            'cameraAligned': 'camera',
            'type': 'baked',                    # 槽位类型，attachable 为动态添加，baked 为使用molang切换显示隐藏
            'category': 'smle_mk3_skill',              # 槽位分类, 自由字符串
            'displayName': 'slot.skill.name', # 槽位的名称，可以使用lang文件中的键
        },
    ]
}