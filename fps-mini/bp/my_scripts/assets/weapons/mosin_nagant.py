...
Asset = {
    ...
    'bolt': {
        ...
        # 手动循环模式相关：
        'shootRestoreTime': 0.4,    # 射击后到可以手拉枪机的时间
        'cuppedPortOpenBolt': 'bolt_open_cup_port',   # 当膛内有实弹拉开枪机（退膛）时播放的动画
        'cuppedPortOpenTime': 1.13,    # 当膛内有实弹拉开枪机（退膛）需要的时间
        # 枪机打开/关闭动画
        'boltOpenAnim': 'bolt_open',        # 枪机打开的动画, 当 cyclemode 为 manual 时会影响换弹动画
        'boltCloseAnim': 'bolt_close',       # 枪机关闭的动画, 当 cyclemode 为 manual 时会影响换弹动画
        'boltOpenTime': 0.33,        # 击发后枪机完全打开的时间
        'boltCloseTime': 0.4,       # 枪机从完全打开回到关闭的时间
        # 通用属性：
    ...
    'feed': {
        ...
        'magazineCapacity': 5,                     # 供弹具容量 (int)
        'ammoType': [ 'bullets.m762_1908' ],     # 兼容的弹药Asset列表
        'reloadModes': [
            {
    ...
    # 枪管组件 - 决定子弹初速和子弹散步
    'barrel': {
        'velocityModifier': 1,       # 子弹的 baseSpeed 乘以这个值就是出膛速度
        'spread': {
    ...
    'features': {
        'shoot': {
            'animation': 'shoot',
            'fireSound': 'gew98.shoot',       # 射击音效，空字符串为不播放
            'emptyFireSound': '',       # 空仓射击音效，空字符串为不播放
        },
        'aim': {     # 基础瞄准功能
            'scale': 1,
            'camera': 'iron_sight',     # 瞄准时相机名称
            'modelScale': 0.3,            # 瞄准时z轴缩放
            'adsIn': 0.3,
            'adsOut': 0.5,
            'vignette': 0.2,            # 暗角范围
    ...
    'slots': [
        ...
        },
        {
            ...
            'attachmentAsset': 'attachments.appearance.mosin_nagant',
            'category': 'appearance_mosin_nagant',                   # 槽位分类, 自由字符串
            'displayName': 'slot.appearance.name',      # 槽位的名称，可以使用lang文件中的键
        },
        {
            'slotId': 'obrez',                     # 槽位 id，在当前武器应该为唯一
            'cameraAligned': 'camera',                  # 选择这个配件时，对齐的相机
            'type': 'baked',                       # 槽位类型，appearance为皮肤和染色
            # 'attachmentAsset': 'attachments.mosin_obrez',
            'category': 'mosin_obrez',                   # 槽位分类, 自由字符串
            'displayName': 'slot.obrez.name',      # 槽位的名称，可以使用lang文件中的键
            'control': 'obrez',
        },
    ]
}