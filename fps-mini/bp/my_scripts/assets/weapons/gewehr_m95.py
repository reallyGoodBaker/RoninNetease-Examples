...
Asset = {
    ...
    'bolt': {
        ...
        'cuppedPortOpenBolt': 'bolt_open',   # 当膛内有实弹拉开枪机（退膛）时播放的动画
        'cuppedPortOpenTime': 0.3,    # 当膛内有实弹拉开枪机（退膛）需要的时间
        # 枪机打开/关闭动画
        ...
        'boltOpenTime': 0.3,        # 击发后枪机完全打开的时间
        'boltCloseTime': 0.25,       # 枪机从完全打开回到关闭的时间
        # 通用属性：
    ...
    'feed': {
        ...
        'ammoType': [ 'bullets.mannlicher850' ],     # 兼容的弹药Asset列表
        'reloadModes': [
            {
                'condition': 'not_full',        # 在子弹没有填满的时候
                'reloadType': 'replace',            # 添加一发, 比如霰弹枪
                'reloadTime': 1,
                'animation': 'reload',
            },
        ]
    ...
    'features': {
        'shoot': {
            'animation': 'shoot',
            'fireSound': 'mannlicher850.shoot',       # 射击音效，空字符串为不播放
    ...
    'slots': [
        ...
        },
        {
            ...
            'attachmentAsset': 'attachments.appearance.gewehr_m95',
            'category': 'appearance_gewehr_m95',                   # 槽位分类, 自由字符串
