...
from ..engine.architect.compact import (
    ...
    vec, modulo, Vector3, normalize, tup,
...
class BulletBase(object):
    ...
    def create(self, asset, velocityModifier, direction, origin):
        ...
        self.pos = origin
        self.velocity = direction * self.initialSpeed # type: Vector3
    ...
    def bulletHitTest(self, clientBullet, pos, v, dt):
        ...
        if not rayCasted:
            clientBullet.updateClientVfx(pos, pos + displacement)
            return
        ...
        clientBullet.updateClientVfx(pos, lastHit and vec(lastHit['hitPos']) or pos + displacement)
    ...
    def handleHitBlock(self, result, clientBullet):
        ...
        if self.speed >= self.penetrate['minPenetrateSpeed']:
            return self.handlePenetrateBlock(blockType, result['pos'], clientBullet)
        else:
            self.flying = False
            return True
    ...
    def handleHitEntity(self, result, clientBullet, vDir):
        ...
        headCenterToRay = pointToLineDist(vec((x, y + halfHeadHeight, z)), vec(result['hitPos']), vDir)
        isHeadShot = headCenterToRay <= halfHeadHeight
        if self.speed >= self.penetrate['minPenetrateSpeed']:
            self.handlePenetrateEntity(entityId, isHeadShot, clientBullet)
        else:
            self.handleKineticPayload(entityId, isHeadShot, clientBullet)
            self.flying = False
            return True


    def handlePenetrateBlock(self, typeId, pos, clientBullet):
        # type: (str, tuple, ClientBulletSystem) -> None
        if typeId in self.penetrateBlocks:
            self.velocity *= self.penetrate['velocityRetentionPerPass']
            self.speed = modulo(self.velocity)
            clientBullet.breakBlock(pos)
            return False
        return True


    def handlePenetrateEntity(self, entityId, isHeadShot, clientBullet):
        # type: (str, bool, ClientBulletSystem) -> None
        self.velocity *= self.penetrate['velocityRetentionPerPass']
        self.speed = modulo(self.velocity)
        ...
        if kinetic:
            ...
            overpenMul = self.penetrate['damageRetentionPerPass']
            damage = baseDamage * damageMul.getValue(self.speed) * overpenMul * headShotMul
            clientBullet.damageEntity(entityId, damage, self.caliber)


    def handleKineticPayload(self, entityId, isHeadShot, clientBullet):
        # type: (str, bool, ClientBulletSystem) -> None
        kinetic =  self.getPayload('kinetic')
        if not kinetic:
            return
        ...
        baseDamage = kinetic['baseDamage']
        damageMul = Asset(kinetic['damageCurve']).load(True) # type: Curve
        damage = baseDamage * damageMul.getValue(self.speed) * headShotMul
        clientBullet.damageEntity(entityId, damage, self.caliber)
...
class ClientBulletSystem(ClientSubsystem):
    ...
    def updateClientVfx(self, start, end):
        if self.debug:
            self.updateDebugBulletTrail(start, end)


    def updateDebugBulletTrail(self, start, end):
        shape = self.drawing.AddLineShape(
            tup(start), tup(end), (0, 1, 0)
        )
        addTimer(0.1, lambda: shape.Remove(), False)


    def breakBlock(self, pos):
        remote.client.call(
            'BulletServerAuthSystem.tryBreakBlock', pos
        )


    def damageEntity(self, target, damage, caliber):
        if self.debug:
